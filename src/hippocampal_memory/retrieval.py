...
class FactRetriever:
    ...
    def probe(
        ...
        where = "WHERE hrr_vector IS NOT NULL AND status != 'archived'"
        params: list = []
        if category:
            where += " AND category = ?"
    ...
    def contradict(
        ...
        where = "WHERE f.hrr_vector IS NOT NULL AND f.status != 'archived'"
        params: list = []
        if category:
    ...
    def _fts_candidates(
        ...
        where_clauses.append("f.status != 'archived'")
